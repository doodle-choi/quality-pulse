...
## 2025-02-25 - [Hardcoded Admin Credentials in NextAuth Configuration]
**Vulnerability:** Hardcoded admin credentials (`admin@example.com` / `admin123`) were present in the `authorize` function of `frontend/src/auth.config.ts`, and pre-filled in the frontend login form.
**Learning:** Keeping hardcoded credentials in authentication logic bypasses dynamic secret injection (like `.env`) and introduces a critical vulnerability where an attacker possessing the source code or unconfigured deployment could log in with full admin privileges.
**Prevention:** Always dynamically inject administrative credentials using environment variables (e.g., `process.env.ADMIN_EMAIL` and `process.env.ADMIN_PASSWORD`) and ensure authentication fails securely if these variables are missing from the environment.
