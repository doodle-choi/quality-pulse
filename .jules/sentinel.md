...
## 2026-03-28 - [XSS via Unsanitized source_url in IssueCard]
**Vulnerability:** The frontend component `IssueCard.tsx` directly rendered `issue.source_url` within an `href` attribute without validation, allowing for potential XSS payloads such as `javascript:alert(1)`.
**Learning:** All user-provided or external database-sourced URLs must be strictly sanitized. Merely mapping properties to React elements in JSX doesn't prevent attribute-based XSS attacks on `href`.
**Prevention:** Implement and enforce a `sanitizeUrl` utility that parses URLs and asserts the protocol is either `http:` or `https:`, defaulting to safe links (e.g., `#`) if parsing fails or invalid protocols are present.
