...
export default async function CrawlerLogsPage() {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight text-text font-headline">Crawler Execution Logs</h1>
      ...
      <div className="grid grid-cols-1 gap-4">
        {logs.length === 0 ? (
          <div className="bg-surface-lowest dark:bg-surface-container rounded-xl p-12 text-center">
            <MaterialIcon name="monitor_heart" className="mx-auto text-text-muted mb-3 !text-5xl opacity-20" />
            <p className="text-text-muted font-medium">No crawl logs found.</p>
            <p className="text-text-muted/60 text-xs mt-1">Run the unified pipeline to see logs here.</p>
          </div>
        ) : (
          logs.map((log) => (
            <Link 
              ...
              className="group bg-surface-lowest dark:bg-surface-container rounded-xl p-5 hover:bg-surface-high/40 transition-all"
            >
                  ...
                  <div className="flex flex-col gap-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-[12px] text-text-muted font-medium">
                      ...
                      <div className="flex items-center gap-1 text-secondary">
                        <MaterialIcon name="bar_chart" size="xs" />
                ...
                <div className="flex items-center gap-6">
                  {log.summary && (
                    <div className="hidden lg:block max-w-[400px]">
                      <p className="text-[12px] text-text-muted line-clamp-2 leading-relaxed italic">
                        &quot;{log.summary}&quot;
                      </p>
                    </div>
                  )}
                  <MaterialIcon name="chevron_right" size="md" className="text-text-muted group-hover:text-primary transition-colors" />
                </div>
              </div>
            </Link>
          ))
